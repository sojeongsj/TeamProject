...
public class MovieDao {
	...
	public int insert (MovieDto movie) throws SQLException {
		...
		ps.setString(3, movie.getMovieDate());
	...
	public int update(MovieDto movie) throws SQLException {
		...
		Connection connection = OracleUtility.getConnection();
	    String sql = "update movie set MovieDate = TO_timestamp(?, 'yyyy-mm-dd'), MovieGrade = ?, MoviePrice = ? where MovieNo = ?";
	    PreparedStatement ps = connection.prepareStatement(sql);
	    ps.setString(1, movie.getMovieDate());
