...
public class Food_Admin_Panel extends JFrame {
	...
	public Food_Admin_Panel() {
		...
		jt = new JTable(dm);
		jt.setBackground(new Color(233, 233, 231));
		jt.setBounds(273, 95, 502, 800);
		...
		JLabel tiTle = new JLabel("CGV 상품 관리 시스템", JLabel.CENTER);
		tiTle.setFont(new Font("굴림", Font.PLAIN, 30));
		tiTle.setBounds(49, 10, 700, 40);
...
//						Timestamp newMovieDate2 = Timestamp.valueOf(newmovieDate1);
	...
	private void fetchMovieData() throws SQLException {
		...
		List<FoodDto1> foods = FoodDAO.getDao().FoodselectAll1();
		...
		for (FoodDto1 food : foods) {
			...
			String[] rowData = { String.valueOf(fOODBUNO), fOODCATE, fOODNAME, String.valueOf(fOODPRICE) };
			dm.addRow(rowData);
		}
