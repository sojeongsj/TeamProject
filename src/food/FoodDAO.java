...
public class FoodDAO {
	...
	//public static FoodDao getDao() { 
		...
		//return dao;
		//}
	public static FoodDAO dao = new FoodDAO();
	public FoodDAO() {}	
	...
	// 관리자 기능 insert 
	public int insert (FoodDTO fd) throws SQLException {
		Connection connection =   OracleUtility.getConnection();
		String sql = "insert into food values(?,?,?,?)";
		PreparedStatement ps = connection.prepareStatement(sql);
		ps.setInt(1, fd.fOODBUNO);
		ps.setString(2, fd.fOODCATE);
		ps.setString(3, fd.fOODNAME);
		ps.setInt(4, fd.fOODPRICE);
	...
	public List<FoodDto1> Foodselectone(int FoodNo) throws SQLException {
		...
		String sql = "select * from food where FoodNo = ? ";
		   ...
		   ResultSet rs = ps.executeQuery();
		   while(rs.next()) {
			   results.add(new FoodDto1(rs.getInt(1),
					   rs.getString(2),
					   rs.getString(3),
					   rs.getInt(4)));
		   }
	...
	public List<FoodDTO> FoodselectAll() throws SQLException {
		Connection connection = OracleUtility.getConnection();
		String sql = "select * from food_buy";
		 PreparedStatement ps = connection.prepareStatement(sql);
		   ...
		   ResultSet rs = ps.executeQuery();
		   while(rs.next()) {
			   results.add(new FoodDTO(rs.getInt(1),
					   rs.getString(2),
					   rs.getString(3),
					   rs.getInt(4),
					   rs.getInt(5),
